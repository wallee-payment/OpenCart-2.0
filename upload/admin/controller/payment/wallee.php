	...
	private function processPostData($shops){
		...
		foreach ($shops as $store) {
			...
			if (($this->request->server['REQUEST_METHOD'] == "POST") && ($this->validateStore($store['id']))) {
				...
				$manual_tasks = $this->model_setting_setting->getSetting(\Wallee\Service\ManualTask::CONFIG_KEY, $store['id']);
				if ($manual_tasks) {
					$manual_tasks = $manual_tasks['value'];
				}
				else {
					$manual_tasks = 0;
				}
				$post[\Wallee\Service\ManualTask::CONFIG_KEY] = $manual_tasks;
				
				// preserve notification url
				$notification_url = $this->model_setting_setting->getSetting('wallee_notification_url', $store['id']);
				if ($notification_url) {
				...
				$post['wallee_notification_url'] = $notification_url;
